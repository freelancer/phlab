...
/**
 * Test cases for PhabricatorTaskComponentProjectEdgeType.
 */
final class PhabricatorTaskComponentProjectEdgeTypeTestCase extends PhabricatorTestCase {

  public function testEdgeConstant(): void {
    $this->assertEqual(10003, PhabricatorTaskComponentProjectEdgeType::EDGECONST);
  }

  public function testConduitKey(): void {
    $edge_type = new PhabricatorTaskComponentProjectEdgeType();
    $this->assertEqual('task.component', $edge_type->getConduitKey());
  }
  ...
  public function testTransactionStrings(): void {
    ...
    // Test add string
    $add_string = $edge_type->getTransactionAddString('admin', 1, 'Test Component');
    $this->assertTrue(is_string($add_string));
    $this->assertTrue(strlen($add_string) > 0);

    // Test remove string
    $remove_string = $edge_type->getTransactionRemoveString('admin', 1, 'Test Component');
    $this->assertTrue(is_string($remove_string));
    $this->assertTrue(strlen($remove_string) > 0);

    // Test edit string
    $edit_string = $edge_type->getTransactionEditString('admin', 2, 1, 'Added', 1, 'Removed');
  ...
  public function testFeedStrings(): void {
    ...
    $add_string = $edge_type->getFeedAddString('admin', 'T123', 1, 'Test Component');
    $this->assertTrue(is_string($add_string));
    $this->assertTrue(strlen($add_string) > 0);

    // Test feed remove string
    $remove_string = $edge_type->getFeedRemoveString('admin', 'T123', 1, 'Test Component');
    $this->assertTrue(is_string($remove_string));
    $this->assertTrue(strlen($remove_string) > 0);

    // Test feed edit string
    $edit_string = $edge_type->getFeedEditString('admin', 'T123', 2, 1, 'Added', 1, 'Removed');
    $this->assertTrue(is_string($edit_string));
    $this->assertTrue(strlen($edit_string) > 0);
  }
